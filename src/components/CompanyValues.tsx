...
const CompanyValues = () => {
  return (
    <section className="py-24 bg-background relative overflow-hidden">
      {/* Geometric Elements */}
      <div className="absolute top-20 right-10 w-72 h-72 border-2 border-border/20 rotate-45" />
        ...
        <div className="grid md:grid-cols-2 gap-8 max-w-5xl mx-auto">
          {values.map((value, index) => (
            <motion.div
              ...
              className="group"
            >
              <div className="flex gap-6 bg-card border-2 border-border p-8 hover:border-accent transition-all h-full">
                {/* Icon with Check Badge */}
                <div className="flex-shrink-0">
                  <div className="w-16 h-16 bg-primary/10 flex items-center justify-center relative">
                    <value.icon className="w-8 h-8 stroke-[2.5] text-primary" />
                ...
                <div className="flex-1">
                  <h3 className="text-xl font-heading font-bold text-primary mb-3 uppercase">
                    {value.title}
                  </h3>
